...
module drive_wheel() {
    ...
    difference() {
        ...
        right(drive_crank_radius) down(0.5)
            cylinder(h = 16, r = drive_pin_diameter / 2 + 0.05);
    };
}
...
module driven_wheel() {
    ...
    up(8) difference() {
        ...
        for (r = [0:360/driven_slot_quantity:360]) rotate([0,0,r + offset])
            ...
            rotate([0,90,0]) linear_extrude(height = slot_center_length, scale=1.0, center = true) square(size = [9, drive_pin_diameter], center = true);
...
module driven_wheel_rounding() {
    offset_length = driven_wheel_radius - allowed_clearance - rotational_clearance;
    offset_side = drive_pin_diameter / 2 + slot_rounding_radius / 2;
...
module drive_wheel_with_axle() {
    difference() {
        ...
        up(-0.5) lego_axle(m=3, hole=true, tolerance=0.17);
    }
}
